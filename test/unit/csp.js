  ...
  it('sends csp headers', function (done) {
    request('http://' + config.host + ':' + config.port, function (err, res, body) {
      expect(res.headers['content-security-policy'])
        .to.equal("default-src 'none';script-src 'self';style-src 'unsafe-inline' 'self' http://fonts.googleapis.com;img-src 'self';connect-src 'self';font-src 'self' data: http://fonts.gstatic.com");
      done();
    });
  });
});
